...
async function doActionInternal(action) {
    try {
        ...
        if (!response.ok && !action.noCorsMode) {
            MainVue.setTooltip(
                i18nService.t("restActionFailed", `${response.statusText} (${response.status})`),
                { revertOnClose: true, timeout: 5000 }
            );
        } else {
            log.debug(`[HTTP2ROS] OK: ${action.restUrl}`);
            return !action.noCorsMode ? await response.text() : '';
        }
    } catch (error) {
        log.error("[HTTP2ROS] ERROR:", error);

        MainVue.setTooltip(
            i18nService.t("restActionFailed", error),
            { revertOnClose: true, timeout: 5000 }
        );
    }
    return '';
